...
public class Practica4 {
    ...
    private static void categoriaPalabrasReservadas(DatosPalabra datosPalabra) {
        String palabra = datosPalabra.getPalabra();
        if (palabra
                .matches("program|begin|end|read|write|if|else|while|repeat|until|int|real|string|bool|var|then|do")) {
        ...
        if (palabra.matches("[(),;:]")) {
            ...
            switch (palabra) {
                ...
                case ";":
                    ...
                    break;
                case ":":
                    datosPalabra.setValorToken(-77); // Se añadio el -77 para el caracter especial : no estaba en la
                                                     // tabla de tokens
                    // System.out.println(datosPalabra);
                    break;
                default:
    ...
    public static void erroresAnalisis() {
        for (DatosPalabra datosPalabra : palabrasDelArchivo) {
            if (datosPalabra.getValorToken() == 0) {
                ...
                if (datosPalabra.getPalabra().charAt(0) == '/' && datosPalabra.getPalabra().charAt(1) == '/'
                        && datosPalabra.getPalabra().charAt(longitud - 1) == '/'
                        && datosPalabra.getPalabra().charAt(longitud - 2) == '/'
                        || datosPalabra.getPalabra().equals(".")) {
                    continue;
                } else {
                    erroresDelArchivo
                            .addLast(new DatosPalabra(datosPalabra.getPalabra().trim(), datosPalabra.getValorToken(),
                                    datosPalabra.getEsIdentificador(), datosPalabra.getPosicion()));
                }
            }
        }
    }
    ...
    public static void escribirArchivoTablaDeErrores() {
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(
                    "C:\\Users\\virgi\\Documents\\ITS\\Semestre 7\\Lenguajes y automatas ii\\Proyecto1\\src\\tabla de errores.txt"));
            for (DatosPalabra datosPalabra : erroresDelArchivo) {
                writer.write("Error en la linea: " + datosPalabra.getPosicion() + ". En la palabra: "
                        + datosPalabra.getPalabra());
                writer.newLine();
            }
            writer.close();
        } catch (IOException e) {
